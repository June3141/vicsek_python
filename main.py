...
delta_path = data_path + "/{0:03d}".format(int(boundary_delta))
noise_path = data_path + "/{0:03d}/{1:03d}".format(int(boundary_delta), int(noise_power * 100))
...
def diff_right_center(r1):
    diff = np.sqrt((r1[0] - boundary_delta / 2) ** 2 + r1[1] ** 2)
    return diff

def diff_left_center(r1):
    diff = np.sqrt((r1[0] + boundary_delta / 2) ** 2 + r1[1] ** 2)
    return diff
...
def particles_init():
    ...
    for i in range(particles_number):
        ...
        particles[i, 0] = x
        particles[i, 1] = y
        particles[i, 2] = 2 * np.pi * np.random.rand() - np.pi
...
def check_boudary(particles_old, particles_new):
    diff_r_list = [diff_right_center(particles_new[i]) for i in range(particles_number)]
    diff_l_list = [diff_left_center(particles_new[i]) for i in range(particles_number)]

    for i in range(particles_number):
        check = 0

        # 右側領域で円境界よりはみ出しているとき
        if diff_r_list[i] > boundary_radius and particles_old[i, 0] > 0:
            x_diff, y_diff, theta_wall = correct_position(particles_old[i], particles_new[i], center_r)
            check = 1

        # 左側領域で円境界よりはみ出しているとき
        elif diff_l_list[i] > boundary_radius and particles_old[i, 0] < 0:
            x_diff, y_diff, theta_wall = correct_position(particles_old[i], particles_new[i], center_l)
            check = 1   
        ...
        if check == 1:
            ...
            particles_new[i, 2] = theta_wall + noise_power * np.random.randn()
...
if __name__ == '__main__':
    ...
    for t in tqdm(range(timestep)):
        ...
        save_path = data_path + \
                    "/{0:03d}/{1:03d}/{2:05d}.dat".format(int(boundary_delta), int(noise_power * 100), t)
